...
parser.add_argument(
    "--real_flag", type=str, default="img", help=("The modality of real path. " "Default to img")
)
parser.add_argument(
    "--fake_flag", type=str, default="txt", help=("The modality of real path. " "Default to txt")
)
parser.add_argument(
    "--real_learn_path", type=str, help=("Paths to the generated images or " "to .npz statistic files")
)
parser.add_argument(
    "--real_forget_path", type=str, help=("Paths to the generated images or " "to .npz statistic files")
)
parser.add_argument(
    "--fake_path", type=str, help=("Paths to the generated images or " "to .npz statistic files")
)
parser.add_argument(
    "--fake_root", type=str, help=("Paths to the generated images or " "to .npz statistic files")
)
...
def main():
    ...
    # --------------开始评估-------------------#
    learn_csv_file = args.fake_root + "/learn.csv"
    forget_csv_file = args.fake_root + "/forget.csv"
    dict_learn = {}
    dict_forget = {}
    if os.path.exists(learn_csv_file):
        print("File already exists: {}".format(learn_csv_file))
        # 读取CSV文件到DataFrame
        df = pd.read_csv(learn_csv_file)
        # 将DataFrame转换为字典
        dict_learn = dict(zip(df['Iteration'], df['Learning_Progress']))
    else:
        print("Loading CLIP model: {}".format(args.clip_model))
        model, preprocess = clip.load(args.clip_model, device=device)
        subdirectories = [d for d in os.listdir(args.fake_root) if os.path.isdir(os.path.join(args.fake_root, d))]
        subdirectories.sort(key=extract_number)
        # ---------------------------------#
        # 生成learn文件
        for subdir in subdirectories:
            loop_count = extract_number(subdir)
            args.fake_path = os.path.join(args.fake_root, subdir, "learn")
            dataset = DummyDataset(
                args.real_learn_path,
                args.fake_path,
                args.real_flag,
                args.fake_flag,
            ...
            dict_learn.update({loop_count: clip_score})
            print(loop_count, "Learn CLIP Score: ", clip_score)
        # 打开 CSV 文件以写入数据
        with open(learn_csv_file, mode="w", newline="") as file:
            writer = csv.writer(file)
            # 写入标题行
            writer.writerow(["Iteration", "Learning_Progress"])
            # 写入数据行
            for loop, learn in dict_learn.items():
                writer.writerow([loop, learn])
    if os.path.exists(forget_csv_file):
        print("File already exists: {}".format(forget_csv_file))
        # 读取CSV文件到DataFrame
        df = pd.read_csv(forget_csv_file)
        # 将DataFrame转换为字典
        dict_forget = dict(zip(df['Iteration'], df['Forgetting_Level']))
    else:
        print("Loading CLIP model: {}".format(args.clip_model))
        model, preprocess = clip.load(args.clip_model, device=device)
        subdirectories = [d for d in os.listdir(args.fake_root) if os.path.isdir(os.path.join(args.fake_root, d))]
        ...
        for subdir in subdirectories:
            loop_count = extract_number(subdir)
            args.fake_path = os.path.join(args.fake_root, subdir, "forget")
            dataset = DummyDataset(
                args.real_forget_path,
                args.fake_path,
                args.real_flag,
                args.fake_flag,
                transform=preprocess,
            ...
            clip_score = clip_score.cpu().item()
            clip_score = round(clip_score, 2)
            dict_forget.update({loop_count: clip_score})
            print(loop_count, "Forget CLIP Score: ", clip_score)
        # 打开 CSV 文件以写入数据
        with open(forget_csv_file, mode="w", newline="") as file:
            writer = csv.writer(file)
            # 写入标题行
            writer.writerow(["Iteration", "Forgetting_Level"])
            # 写入数据行
            for loop, learn in dict_forget.items():
                writer.writerow([loop, learn])

    x = list(dict_learn.keys())
    learn_y = list(dict_learn.values())
    forget_y=list(dict_forget.values())
    # 创建折线图
    plt.figure(figsize=(10, 6))
    plt.plot(x, learn_y, marker='o', linestyle='-', color='r', linewidth=2, label='Learning Progress')
    plt.plot(x, forget_y, marker='*', linestyle='--', color='b', linewidth=2, label='Forgetting Level')
    ...
    # 设置图的标题
    plt.title(args.experiment_name+' Learning and Forgetting Curves Across Iterations')

    # 显示网格
    plt.grid(True)
    # 设置横轴的间隔为20
    plt.xticks(range(min(x), max(x) + 1, 20))
    # 添加图例
    plt.legend()
    # 将图形保存为图片文件
    plt.savefig(args.fake_root+'/'+args.experiment_name+'_learning_forgetting_plot.png')
    # 显示图
    plt.show()
